...
Public releases are versioned with Git tags in the format `vX.Y.Z`.

Keep `package.json` `version` aligned with the latest public release tag.

Between releases, keep `package.json` on the last published public version until the next release is prepared.

Use `CHANGELOG.md` as the human-readable release history for public versions and unreleased changes.

After dependency updates, build configuration changes, or release metadata changes, run a fresh `pnpm build` before `pnpm start`.

Do not treat `.next/standalone` output as current unless it was generated by a fresh build from the current workspace state.
